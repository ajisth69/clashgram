...
const PRODUCTION_URL = 'https://clashgram.pages.dev';
...
export default defineConfig(({ mode }): UserConfig => {
  ...
  const defaultAppTitle = `Clashgram${appEnv !== 'production' ? ' Beta' : ''}`;
  ...
  // Calculate git revision details for global definition
  const branch = HEAD || getGitValue(DIR_NAME, ['rev-parse', '--abbrev-ref', 'HEAD']);
  const commit = getGitValue(DIR_NAME, ['rev-parse', '--short=7', 'HEAD']);
  ...
  return {
    ...
    define: {
      ...
      'process.env.APP_MOCKED_CLIENT': JSON.stringify(appMockedClient),
      'process.env.APP_NAME': JSON.stringify(env.APP_NAME || ''),
      'process.env.APP_TITLE': JSON.stringify(appTitle),
      'process.env.TEST_SESSION': JSON.stringify(env.TEST_SESSION || ''),
    },
    resolve: {
      tsconfigPaths: true,
      alias: [
        ...(appMockedClient === '1' ? [{
          find: /^(?:\.\/client|(?:\.\.\/)*lib\/gramjs\/client)\/ClashgramClient$/,
