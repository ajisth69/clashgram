...
addActionHandler('apiUpdate', (global, actions, update): ActionReturnType => {
  switch (update['@type']) {
    ...
    case 'updateMessage': {
      ...
      const { clashgramAntiEdit } = selectSharedSettings(global);
      if (clashgramAntiEdit) {
        const oldText = currentMessage.content?.text?.text;
        const newText = message.content?.text?.text;
        if (oldText && newText !== undefined && newText !== oldText) {
          message.clashgramOriginalText = currentMessage.clashgramOriginalText || oldText;
          message.clashgramEdited = true;
          saveClashgramMessage(chatId, id, message as ApiMessage);
        }
      }
    ...
    case 'updateMessageSendSucceeded': {
      ...
      // For some reason Clashgram requires to manually mark outgoing thread messages read
      Object.values(global.byTabId).forEach(({ id: tabId }) => {
    ...
    case 'updateThreadReadState': {
      ...
      // Real-time bypass: if lastReadOutboxMessageId is updated in a private chat, set readDate for matching messages
      if (readState.lastReadOutboxMessageId && isUserId(chatId)) {
        const messagesById = selectChatMessages(global, chatId);
        if (messagesById) {
          const maxId = readState.lastReadOutboxMessageId;
          Object.keys(messagesById).forEach((idStr) => {
            const id = Number(idStr);
            if (id <= maxId) {
              const msg = messagesById[id];
              if (msg && isOwnMessage(msg) && !msg.readDate) {
                global = updateChatMessage(global, chatId, id, {
                  readDate: Math.round(Date.now() / 1000),
                });
              }
            }
          });
    ...
    case 'resetMessages': {
      ...
      if (messagesById && !isUserId(chatId)) {
        const tabId = getCurrentTabId();
        global = deleteChatMessages(global, chatId, Object.keys(messagesById).map(Number), true);
        setGlobal(global);
...
export function updateWithLocalMedia(
  ...
  if (currentMessage && messageUpdate.content && !isLocalMessageId(id)) {
    ...
    const isSelfDestructing = currentContent?.ttlSeconds !== undefined;
    const isUpdateExpired = messageUpdate.content?.action?.type === 'expired';

    if (isSelfDestructing && isUpdateExpired) {
      const msgDate = currentMessage.date;
      const isUnder7Days = (getServerTime() - msgDate) < 7 * 24 * 60 * 60; // 7 days in seconds
