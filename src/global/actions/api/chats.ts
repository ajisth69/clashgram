  ...
  if (id) {
    ...
    if (isLocked && !isUnlocked) {
      actions.openClashgramPasscodeModal({
        type: 'chat',
        targetId: id,
        pendingAction: { type: 'openChat', payload },
        tabId,
      });
      return undefined;
    }
...
addActionHandler('markChatMessagesRead', async (global, actions, payload): Promise<void> => {
  ...
  if (!chat.isForum) {
    ...
    // Always perform local updates to clear unread counts immediately in the UI (essential when Ghost Mode is active)
    global = getGlobal();
    const lastMessageId = selectChatLastMessageId(global, id);
    if (lastMessageId) {
      global = replaceThreadReadStateParam(global, id, MAIN_THREAD_ID, 'lastReadInboxMessageId', Number(lastMessageId));
    }
    global = replaceThreadReadStateParam(global, id, MAIN_THREAD_ID, 'unreadCount', 0);
    global = replaceThreadReadStateParam(global, id, MAIN_THREAD_ID, 'hasUnreadMark', undefined);
    setGlobal(global);
...
addActionHandler('openClashgramLink', async (global, actions, payload): Promise<void> => {
  const {
    url,
    shouldIgnoreCache,
    linkContext,
...
addActionHandler('setActiveChatFolder', (global, actions, payload): ActionReturnType => {
  ...
  if (activeChatFolder !== undefined) {
    ...
    if (isLocked && !isUnlocked) {
      actions.openClashgramPasscodeModal({
        type: 'folder',
        targetId: activeChatFolder,
        pendingAction: { type: 'setActiveChatFolder', payload },
        tabId,
      });
      return undefined;
    }
...
async function loadChats(
  ...
  if (isFirstBatch) {
    if (shouldIgnorePagination) {
      const currentIds = global.chats.listIds[listType] || [];
      const mergedIds = [...chatIds, ...currentIds.filter((id) => !chatIds.includes(id))];
      global = replaceChatListIds(global, listType, mergedIds);
    } else {
