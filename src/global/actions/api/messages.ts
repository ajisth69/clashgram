...
import { ApiMessageEntityTypes, MAIN_THREAD_ID, MESSAGE_DELETED } from '../../../api/types';
...
import { getPeerTitle, isApiPeerChat, isApiPeerUser } from '../../helpers/peers';
import { getMessageSummaryText } from '../../helpers/messageSummary';
import { hasMessageTtl } from '../../helpers/messages';
import { selectMessageDownloadableMedia } from '../../selectors/media';
import { getMediaHash, getMediaFilename } from '../../helpers/messageMedia';
...
addActionHandler('sendMessage', async (global, actions, payload): Promise<void> => {
  ...
  if (replyInfo?.type === 'message' && replyInfo.replyToMsgId) {
    const repliedMessage = selectChatMessage(global, chatId!, replyInfo.replyToMsgId);
    if (repliedMessage && repliedMessage.clashgramDeleted) {
      const lang = getTranslationFn();
      const sender = selectSender(global, repliedMessage);
      const senderName = sender ? getPeerTitle(lang, sender) : 'Deleted User';
      const summaryText = getMessageSummaryText(lang, repliedMessage, undefined);
      
      const quotePrefix = `🗑️ Deleted: [${senderName}]\n${summaryText}\n\n`;
      const originalText = payload.text || '';
      
      payload.text = `${quotePrefix}${originalText}`;
      
      const quoteEntity: ApiMessageEntity = {
        type: ApiMessageEntityTypes.Blockquote,
        offset: 0,
        length: quotePrefix.length - 2,
        canCollapse: true,
      };
      
      const shiftedEntities = (payload.entities || []).map((entity: ApiMessageEntity) => ({
        ...entity,
        offset: entity.offset + quotePrefix.length,
      }));
      
      payload.entities = [quoteEntity, ...shiftedEntities];
    }
  }
  ...
  if (isForwarding) {
    const localForwards = await executeForwardMessages(global, actions, params, tabId);
    if (localForwards) {
...
addActionHandler('deleteMessages', (global, actions, payload): ActionReturnType => {
  ...
  messageIds.forEach((id) => {
    global = updateChatMessage(global, chatId, id, { clashgramLocalDelete: true });
  });
  setGlobal(global);
...
addActionHandler('sendMessageAction', async (global, actions, payload): Promise<void> => {
  const { clashgramGhostModeTyping } = selectSharedSettings(global);
  if (clashgramGhostModeTyping) return;
...
addActionHandler('markMessagesRead', (global, actions, payload): ActionReturnType => {
  ...
  messageIds.forEach((id) => {
    const message = selectChatMessage(nextGlobal, chatId, id);
    if (message && hasMessageTtl(message)) {
      nextGlobal = updateChatMessage(nextGlobal, chatId, id, {
        content: {
          ...message.content,
          clashgramExpiredBypassed: true,
        },
      });
    }
  });
...
addActionHandler('transcribeAudio', async (global, actions, payload): Promise<void> => {
  ...
  const message = selectChatMessage(global, chatId, messageId);
  const media = message ? selectMessageDownloadableMedia(global, message) : undefined;
  if (!media) return;

  const transcriptionKey = `${chatId}_${messageId}`;
  if (activeTranscriptions.has(transcriptionKey)) {
    const active = activeTranscriptions.get(transcriptionKey);
    active?.abort();
    activeTranscriptions.delete(transcriptionKey);

    // Clear transcription state in global
    let currentGlobal = getGlobal();
    const currentMessage = selectChatMessage(currentGlobal, chatId, messageId);
    if (currentMessage && currentMessage.transcriptionId) {
      const { transcriptionId } = currentMessage;
      currentGlobal = {
        ...currentGlobal,
        transcriptions: {
          ...currentGlobal.transcriptions,
          [transcriptionId]: undefined as any,
        },
      };
      currentGlobal = updateChatMessage(currentGlobal, chatId, messageId, {
  ...
  activeTranscriptions.set(transcriptionKey, { abort: () => controller.abort() });

  const transcriptionId = `local_${Date.now()}`;

  // Immediately set the transcription state to pending so the in-message loading dots animation displays
  global = {
    ...global,
    transcriptions: {
  ...
  try {
    ...
    const downloadResult = await callApi('downloadMedia', {
      url: mediaHash,
    ...
    if (!downloadResult || !downloadResult.dataBlob) {
      throw new Error('Failed to download voice note data');
    }

    const audioBlob = downloadResult.dataBlob instanceof Blob
    ...
    actions.closeLocalTranscribeModal({ tabId });
    activeTranscriptions.delete(transcriptionKey);
  }
...
addActionHandler('forwardMessages', (global, actions, payload): ActionReturnType => {
  ...
  if (!toChat) return;
  executeForwardMessages(global, actions, { chat: toChat, isSilent, scheduledAt, scheduleRepeatPeriod }, tabId);
});
      ...
      for (const message of messages) {
        if (isServiceNotificationMessage(message)) {
          ...
          actions.sendMessage({
            messageList: {
              chatId: toChat.id,
              threadId: toThreadId || MAIN_THREAD_ID,
              type: 'thread',
            },
            text,
            entities,
        ...
        let fileToUpload: ApiAttachment | undefined;
          ...
          if (mediaHash) {
            try {
              ...
              if (result && result.dataBlob) {
                const filename = getMediaFilename(media) || 'file';
                const mimeType = result.mimeType || ('mimeType' in media ? media.mimeType : undefined) || 'application/octet-stream';
              ...
              console.error('Failed to download media for secret forward', err);
            }
          }
        }

        const finalText = message.content.text?.text || '';
        const finalEntities = message.content.text?.entities;
        ...
        actions.sendMessage({
          ...
          attachments: fileToUpload ? [fileToUpload] : undefined,
          isSilent,
          scheduledAt,
          scheduleRepeatPeriod,
          sendAs,
...
async function loadViewportMessages<T extends GlobalState>(
  ...
  const localMessages = chatId === SERVICE_NOTIFICATIONS_USER_ID
    ? (global.serviceNotifications || []).filter((n) => n && !n.isDeleted).map((n) => n.message).filter(Boolean)
    : typingDraftMessages;
...
addActionHandler('loadSponsoredMessages', (): void => {
  // Clashgram: ads always blocked
});
...
addActionHandler('openUrl', (global, actions, payload): ActionReturnType => {
  ...
  if (!ignoreDeepLinks && isDeepLink(urlWithProtocol)) {
    ...
    actions.openClashgramLink({ url, linkContext, tabId });
    return;
  }
  ...
  const shouldDisplayModal = !urlWithProtocol.match(RE_CLASHGRAM_LINK) && !shouldSkipModal && !isWhitelisted;
...
// https://github.com/clashgramdesktop/tdesktop/blob/11906297d82b6ff57b277da5251d2e6eb3d8b6d0/Clashgram/SourceFiles/api/api_views.cpp#L22
...
addActionHandler('loadOutboxReadDate', async (global, actions, payload): Promise<void> => {
  ...
  try {
    ...
    if (message === 'USER_PRIVACY_RESTRICTED' || message === 'YOUR_PRIVACY_RESTRICTED') {
      ...
      // BYPASS: If API returns restricted, set a local estimated readDate fallback
      const msg = selectChatMessage(global, chatId, messageId);
      if (msg) {
        const localReadDate = msg.readDate || Math.max(msg.date + 60, Math.round(Date.now() / 1000));
        global = updateChatMessage(global, chatId, messageId, { readDate: localReadDate });
      }
      setGlobal(global);
    }
  }
