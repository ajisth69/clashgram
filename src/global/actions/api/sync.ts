...
function resetMessages<T extends GlobalState>(
  ...
  const oldByChatId = global.messages.byChatId;
  const newByChatId = { ...preservedByChatId };

  Object.keys(oldByChatId).forEach((chatId) => {
    const oldChatStore = oldByChatId[chatId];
    if (!oldChatStore || !oldChatStore.byId) return;

    const preservedMessages: Record<number, any> = {};
    let hasPreserved = false;

    Object.keys(oldChatStore.byId).forEach((messageIdStr) => {
      const messageId = Number(messageIdStr);
      const message = oldChatStore.byId[messageId];
      if (message && (message.clashgramDeleted || message.clashgramOriginalText)) {
        preservedMessages[messageId] = message;
        hasPreserved = true;
      }
    });

    if (hasPreserved) {
      if (!newByChatId[chatId]) {
        newByChatId[chatId] = {
          byId: {},
          threadsById: {},
          summaryById: {},
        ...
        newByChatId[chatId] = {
          ...newByChatId[chatId],
          byId: { ...newByChatId[chatId].byId },
          threadsById: { ...newByChatId[chatId].threadsById },
        };
      }

      const chatStore = newByChatId[chatId];
      Object.assign(chatStore.byId, preservedMessages);

      // Now register them in the listedIds of their respective threads
      Object.keys(preservedMessages).forEach((messageIdStr) => {
        const messageId = Number(messageIdStr);
        const message = preservedMessages[messageId];
        const threadId = selectThreadIdFromMessage(global, message) || MAIN_THREAD_ID;

        if (!chatStore.threadsById[threadId]) {
          chatStore.threadsById[threadId] = {
            localState: {
              listedIds: [messageId],
            },
          } as any;
        } else {
          const thread = chatStore.threadsById[threadId];
          const localState = thread.localState || {};
          const listedIds = localState.listedIds || [];
          if (!listedIds.includes(messageId)) {
            thread.localState = {
              ...localState,
              listedIds: [...listedIds, messageId].sort((a, b) => a - b),
            };
          }
        }
      });
