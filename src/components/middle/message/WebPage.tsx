...
const WEBPAGE_STORY_TYPE = 'clashgram_story';
const WEBPAGE_GIFT_TYPE = 'clashgram_nft';
const WEBPAGE_AUCTION_TYPE = 'clashgram_auction';
...
const WebPage = ({
  ...
  const { openUrl, openClashgramLink } = getActions();
  ...
  const handleOpenClashgramLink = useLastCallback(() => {
    openClashgramLink({
      url: webPage.url!,
    });
  });
  ...
  const resultType = stickers?.isEmoji ? 'clashgram_emojiset' : type;
  ...
  function renderQuickButton() {
    return (
      <Button
        ...
        onClick={handleOpenClashgramLink}
      >
