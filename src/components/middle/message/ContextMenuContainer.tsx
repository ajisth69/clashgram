...
const ContextMenuContainer: FC<OwnProps & StateProps> = ({
  ...
  useEffect(() => {
    if (canLoadReadDate && isOpen && !message.readDate) {
  ...
  const handleAppendTodoList = useLastCallback(() => {
    if (!isCurrentUserPremium) {
      showNotification({
        message: lang('SubscribeToClashgramPremiumForAppendToDo'),
        action: {
  ...
  const handleSearchByUser = useLastCallback(() => {
    if (!message.senderId) return;
    getActions().openMiddleSearch({ fromPeerId: message.senderId });
    closeMenu();
  });
  ...
  return (
    <div ref={containerRef} className={buildClassName('ContextMenuContainer', className)}>
      <MessageContextMenu
        ...
        canPin={canPin}
        canReport={canReport}
        canSearchByUser={!isPrivate && Boolean(message.senderId)}
        ...
        onSearchByUser={handleSearchByUser}
...
export default memo(withGlobal<OwnProps>(
  (global, { message, messageListType, detectedLanguage }): Complete<StateProps> => {
    ...
    const transcribedText = transcriptionId !== undefined ? global.transcriptions[transcriptionId]?.text : undefined;
