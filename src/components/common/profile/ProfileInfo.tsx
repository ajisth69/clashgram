  ...
  return (
    ...
    >
      ...
      >
        ...
        {peerId && (
          <div
            className="peer-id-badge"
            style={`font-size: 0.8125rem; color: ${isExpanded ? 'rgba(255, 255, 255, 0.7)' : 'var(--color-text-secondary)'}; margin-top: 0.25rem; cursor: pointer; user-select: all; align-self: ${isExpanded ? 'flex-start' : 'center'}; pointer-events: auto;`}
            onClick={(e) => {
              e.stopPropagation();
              copyTextToClipboard(peerId);
              showNotification({ message: 'ID copied to clipboard' });
            }}
          >
            ID: {peerId}
          </div>
