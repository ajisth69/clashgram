...
const ChatExtra = ({
  ...
  const handleCallClick = useLastCallback((e: any) => {
    stopEvent(e);
    requestMasterAndRequestCall({ userId: peerId! });
  });
  ...
  return (
    <div className={buildClassName('ChatExtra', className)} style={style || createVtnStyle('chatExtra')}>
      ...
      <Wrapper>
        {peerId && (
          <ListItem
            icon={userId ? 'user' : 'group'}
            multiline
            narrow
            ripple
            onClick={() => copy(peerId, userId ? 'User ID' : 'Group ID')}
            style={createVtnStyle('peerId')}
          >
            <span className="title" dir={lang.isRtl ? 'rtl' : undefined}>{peerId}</span>
            <span className="subtitle">{userId ? 'User ID' : 'Group ID'}</span>
          </ListItem>
        )}
        {Boolean(formattedNumber?.length) && (
          <ListItem
            ...
            rightElement={canCall && (
              <Button
                round
                color="translucent"
                size="smaller"
                onClick={handleCallClick}
                ariaLabel="Call via Clashgram"
              >
                <Icon name="phone" />
              </Button>
            )}
...
export default memo(withGlobal<OwnProps>(
  (global, { chatOrUserId, isSavedDialog }): Complete<StateProps> => {
    ...
    const isPrivate = chatOrUserId ? isUserId(chatOrUserId) : false;
    const isChatWithSelf = chatOrUserId ? selectIsChatWithSelf(global, chatOrUserId) : false;
    const bot = chatOrUserId ? selectBot(global, chatOrUserId) : undefined;
    const isBot = Boolean(bot);
    const isSupport = chat?.isSupport || user?.isSupport;
    const canCall = ARE_CALLS_SUPPORTED && isPrivate && !isChatWithSelf && !isBot && !isSupport
      && !isAnonymousForwardsChat(chatOrUserId);
