...
import { processVoicePCM, encodePCMToOgg } from '../../util/voiceRecording';
...
const Composer = ({
  ...
  const [pendingVoiceRecord, setPendingVoiceRecord] = useState<{
    record: any;
    isSilent: boolean;
    scheduledAt?: number;
  ...
  const [isVoiceChangerModalOpen, setIsVoiceChangerModalOpen] = useState(false);
  const [isVoiceChangerProcessing, setIsVoiceChangerProcessing] = useState(false);
  ...
  const handleVoiceChangerModeSelect = useLastCallback(async (mode: string) => {
    if (!pendingVoiceRecord) return;
    const { record, isSilent, scheduledAt, scheduleRepeatPeriod, ttlSeconds } = pendingVoiceRecord;

    setIsVoiceChangerProcessing(true);
    try {
      let finalBlob = record.blob;
      if (mode !== 'original' && record.rawPCM && record.sampleRate) {
        const processedPCM = processVoicePCM(record.rawPCM, record.sampleRate, mode);
        finalBlob = await encodePCMToOgg(processedPCM, record.sampleRate);
      }

      const attachment = await buildAttachment(
        VOICE_RECORDING_FILENAME,
        finalBlob,
        { voice: { duration: record.duration, waveform: record.waveform }, ttlSeconds },
      );

      handleSendCore([attachment], isSilent, scheduledAt, scheduleRepeatPeriod);
      setIsVoiceChangerModalOpen(false);
      setPendingVoiceRecord(undefined);
    } catch (err) {
      console.error(err);
    } finally {
      setIsVoiceChangerProcessing(false);
    }
  });

  const handleVoiceChangerCancel = useLastCallback(() => {
    setIsVoiceChangerModalOpen(false);
    setPendingVoiceRecord(undefined);
  });
  ...
  const handleSend = useLastCallback(async (
    ...
    if (activeVoiceRecording) {
      ...
      if (record) {
        if (clashgramVoiceChangerEnabled && record.rawPCM) {
          setPendingVoiceRecord({
            record,
            isSilent,
            scheduledAt,
            scheduleRepeatPeriod,
          ...
          setIsVoiceChangerModalOpen(true);
          return;
        }
  ...
  const handleTodoListCreate = useLastCallback(() => {
    if (!isCurrentUserPremium) {
      showNotification({
        message: lang('SubscribeToClashgramPremiumForCreateToDo'),
        action: {
          action: 'openPremiumModal',
  ...
  return (
    <div className={fullClassName}>
      ...
      />
      <Modal
        isOpen={isVoiceChangerModalOpen}
        onClose={handleVoiceChangerCancel}
        title="Voice Changer Effect"
        className="voice-changer-modal"
        noBackdropClose={isVoiceChangerProcessing}
        hasCloseButton={!isVoiceChangerProcessing}
      >
        {isVoiceChangerProcessing ? (
          <div className="voice-changer-processing" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 2rem 0; gap: 1rem;">
            <Spinner color="blue" />
            <span style="font-weight: 500; font-size: 1.1rem; color: var(--color-text);">Applying realistic voice effect...</span>
            <span style="font-size: 0.9rem; color: var(--color-text-secondary); text-align: center;">Locally modulating audio waves & encoding Ogg/Opus offline</span>
          </div>
        ) : (
          <div className="voice-changer-options" style="display: flex; flex-direction: column; gap: 0.75rem; padding: 0.5rem 0 1rem 0;">
            <p style="font-size: 0.95rem; color: var(--color-text-secondary); margin-bottom: 0.75rem;">
              Select a realistic modulation effect to transform your voice before sending:
            </p>
            <Button
              color="secondary"
              onClick={() => handleVoiceChangerModeSelect('original')}
              style="justify-content: flex-start; text-align: left; padding: 0.8rem 1.2rem; border-radius: 0.5rem; font-weight: 500; font-size: 0.95rem;"
            >
              <Icon name="microphone-alt" style="margin-right: 0.8rem; font-size: 1.2rem; color: var(--color-primary);" />
              Original (No Effect)
            </Button>
            <Button
              color="secondary"
              onClick={() => handleVoiceChangerModeSelect('robotic')}
              style="justify-content: flex-start; text-align: left; padding: 0.8rem 1.2rem; border-radius: 0.5rem; font-weight: 500; font-size: 0.95rem;"
            >
              <Icon name="message" style="margin-right: 0.8rem; font-size: 1.2rem; color: #ff5722;" />
              Robotic Voice (Metallic carrier modulator)
            </Button>
            <Button
              color="secondary"
              onClick={() => handleVoiceChangerModeSelect('deep')}
              style="justify-content: flex-start; text-align: left; padding: 0.8rem 1.2rem; border-radius: 0.5rem; font-weight: 500; font-size: 0.95rem;"
            >
              <Icon name="play" style="margin-right: 0.8rem; font-size: 1.2rem; color: #3f51b5;" />
              Deep Voice (Warm radio bass tone)
            </Button>
            <Button
              color="secondary"
              onClick={() => handleVoiceChangerModeSelect('girl')}
              style="justify-content: flex-start; text-align: left; padding: 0.8rem 1.2rem; border-radius: 0.5rem; font-weight: 500; font-size: 0.95rem;"
            >
              <Icon name="heart" style="margin-right: 0.8rem; font-size: 1.2rem; color: #e91e63;" />
              Girl Voice (Higher feminine frequency pitch)
            </Button>
            <Button
              color="secondary"
              onClick={() => handleVoiceChangerModeSelect('chipmunk')}
              style="justify-content: flex-start; text-align: left; padding: 0.8rem 1.2rem; border-radius: 0.5rem; font-weight: 500; font-size: 0.95rem;"
            >
              <Icon name="smile" style="margin-right: 0.8rem; font-size: 1.2rem; color: #4caf50;" />
              Chipmunk Voice (Ultra-pitched playful effect)
            </Button>
            <Button
              color="secondary"
              onClick={() => handleVoiceChangerModeSelect('echo')}
              style="justify-content: flex-start; text-align: left; padding: 0.8rem 1.2rem; border-radius: 0.5rem; font-weight: 500; font-size: 0.95rem;"
            >
              <Icon name="schedule" style="margin-right: 0.8rem; font-size: 1.2rem; color: #00bcd4;" />
              Echo / Cave (Surrounding feedback delay)
            </Button>
            
            <div style="height: 1px; background: var(--color-borders); margin: 0.5rem 0;" />
            
            <Button
              color="danger"
              onClick={handleVoiceChangerCancel}
              style="border-radius: 0.5rem; font-weight: 500; font-size: 0.95rem;"
            >
              Discard Recording
            </Button>
          </div>
