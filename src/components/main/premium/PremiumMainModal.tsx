...
const PremiumMainModal: FC<OwnProps & StateProps> = ({
  ...
  const {
    closePremiumModal, openInvoice, requestConfetti, openClashgramLink, loadStickers, openStickerSet,
  } = getActions();
  ...
  function getHeaderText() {
    ...
    return renderText(
      fromUser
        ? lang('ClashgramPremiumUserDialogTitle', { user: getUserFullName(fromUser) })
        : lang(isPremium ? 'ClashgramPremiumSubscribedTitle' : 'ClashgramPremium'),
      ['simple_markdown', 'emoji'],
    );
  }
  ...
  function getHeaderDescription() {
    ...
    if (isGift) {
      return fromUser?.id === currentUserId
        ? lang('ClashgramPremiumUserGiftedPremiumOutboundDialogSubtitle', { user: getUserFullName(toUser) })
        : lang('ClashgramPremiumUserGiftedPremiumDialogSubtitle');
    }

    if (fromUserStatusSet) {
      return lang('ClashgramPremiumUserStatusDialogSubtitle');
    }

    return fromUser
      ? lang('ClashgramPremiumUserDialogSubtitle')
      : lang(isPremium ? 'ClashgramPremiumSubscribedSubtitle' : 'ClashgramPremiumSubtitle');
  }
