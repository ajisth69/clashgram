...
/// NOTE: clashgram returns sign source, while webrtc uses unsign source internally
/// unsign => sign
export function toClashgramSource(source: number) {
  return source << 0;
}

/// NOTE: clashgram returns sign source, while webrtc uses unsign source internally
/// sign => unsign
export function fromClashgramSource(source: number) {
  return source >>> 0;
}
