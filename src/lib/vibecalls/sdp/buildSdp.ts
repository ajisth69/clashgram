...
import { fromClashgramSource } from '../utils';
  ...
  const addSsrcEntry = (entry: Ssrc) => {
    ...
    entry.sourceGroups.forEach((sourceGroup) => {
      if (sourceGroup.semantics) {
        add(`a=ssrc-group:${sourceGroup.semantics} ${sourceGroup.sources.map(fromClashgramSource).join(' ')}`);
      }
      sourceGroup.sources.forEach((ssrcClashgram) => {
        const ssrc = fromClashgramSource(ssrcClashgram);
