...
class ClashgramClient {
  static DEFAULT_OPTIONS: Partial<ClashgramClientParams> = {
  ...
  private _args: ClashgramClientParams;
  ...
  constructor(
    ...
    apiId?: number,
    apiHash?: string,
    opts: Partial<ClashgramClientParams> = ClashgramClient.DEFAULT_OPTIONS,
  ) {
    if (!apiId || !apiHash || !Number.isFinite(apiId)) {
      throw Error('Your API ID or Hash are invalid. Please read "Requirements" on README.md');
    }
    const args = { ...ClashgramClient.DEFAULT_OPTIONS, ...opts } as ClashgramClientParams;
    this.apiId = apiId;
  ...
  /**
     * Connects to the Clashgram servers, executing authentication if required.
     * Note that authenticating to the Clashgram servers is not the same as authenticating
  ...
  async setForceHttpTransport(forceHttpTransport: boolean) {
    if (forceHttpTransport) {
      this._log.warn('HTTP transport forcing is disabled; keeping WSS transport');
    }
  ...
  async setAllowHttpTransport(allowHttpTransport: boolean) {
    if (allowHttpTransport) {
      this._log.warn('HTTP fallback transport is disabled; keeping WSS transport');
    }

    this._shouldAllowHttpTransport = false;
    this.disconnect();
    this._sender = undefined;
    await this.connect();
  }
    ...
    while (!this._destroyed) {
      ...
      try {
        ...
        if (!lastInterval || lastInterval < PING_INTERVAL_TO_WAKE_UP) {
          ...
          // Returning from background: send wake-up ping first.
          // Do not preemptively dispatch 'disconnected' because the socket is likely still alive.
      ...
      // for Clashgram to keep delivering updates, otherwise they will
  ...
  /**
     * Disconnects from the Clashgram server
     * @returns {Promise<void>}
     */
  disconnect() {
  ...
  async _connectSender(sender: MTProtoSender, dcId: number, index?: number, isPremium = false) {
    ...
    if (!hasAuthKey) {
      if (this._waitingForAuthKey[dcId]) {
        ...
        const loadedKey = authKey?.getKey();
        if (loadedKey) {
          await sender.authKey.setKey(loadedKey);
          hasAuthKey = true;
        }
      } else {
  ...
  async _downloadWebDocument(media: Api.TypeWebDocument) {
    ...
    try {
      ...
      // the file is no longer saved in clashgram's cache.
  ...
  // region Invoking Clashgram request
  /**
    ...
    for (attempt = 0; attempt < this._requestRetries; attempt++) {
      ...
      try {
        ...
        if (e instanceof ServerError
          ...
          this._log.warn(`Clashgram is having internal issues ${e.constructor.name}`);
          await sleep(2000);
...
export default ClashgramClient;
