...
import type ClashgramClient from './ClashgramClient';
import type { Update } from './ClashgramClient';
...
export function signInUserWithPreferredMethod(
  client: ClashgramClient, apiCredentials: ApiCredentials, authParams: UserAuthParams,
...
export async function checkAuthorization(client: ClashgramClient, shouldThrow = false) {
  try {
  ...
  if (isRegistrationRequired) {
    ...
    while (1) {
      try {
        ...
        if (termsOfService) {
          // This is a violation of Clashgram rules: the user should be presented with and accept TOS.
...
export async function obtainPasskeyLoginOption(
  client: ClashgramClient, apiCredentials: ApiCredentials,
...
async function sendCode(
  client: ClashgramClient, apiCredentials: ApiCredentials, phoneNumber: string, forceSMS = false,
): Promise<{
...
async function signInWithPassword(
  client: ClashgramClient, apiCredentials: ApiCredentials, authParams: UserAuthParams, noReset = false,
): Promise<Api.TypeUser> {
  // eslint-disable-next-line no-constant-condition
  while (1) {
    try {
...
async function signInBot(client: ClashgramClient, apiCredentials: ApiCredentials, authParams: BotAuthParams) {
  const { apiId, apiHash } = apiCredentials;
