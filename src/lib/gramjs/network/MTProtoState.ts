...
export default class MTProtoState {
  ...
  /**
   * Calculate the key based on Clashgram guidelines, specifying whether it's the client or not
   * @param authKey
   * @param msgKey
   * @param client
  ...
  async encryptMessageData(data: Buffer<ArrayBuffer>): Promise<Buffer<ArrayBuffer>> {
    ...
    const currentKeyId = this.authKey.keyId;

    if (this.salt === undefined || currentKeyId === undefined || currentKeyId === null) {
    ...
    if (this._isCall) {
      ...
      const keyId = readBufferFromBigInt(currentKeyId, 8);
  ...
  async decryptMessageData(body: Buffer<ArrayBuffer>) {
    ...
    if (!this._isCall) {
      ...
      if (currentKeyId === undefined || currentKeyId === null || keyId !== currentKeyId) {
    ...
    if (this._isCall) {
      ...
      // We only check for objects that clashgram has returned to us (Updates) not ones we send.
  ...
  updateTimeOffset(correctMsgId: bigint) {
    ...
    if (this.timeOffset !== old) {
      ...
      this._log.debug(
        ...
        `Updated time offset and reset session (old offset ${old}, bad ${bad.toString()}, good ${correctMsgId.toString()}, new ${this.timeOffset})`,
      );
    }
