...
export function toSignedLittleBuffer(big: bigint | number | string, number = 8) {
  const bigIntValue = typeof big === 'bigint' ? big : BigInt(big);
  const buffer = Buffer.allocUnsafe(number);

  // Use Buffer method for 8-byte buffers
  if (number === 8) {
    buffer.writeBigInt64LE(bigIntValue);
    return buffer;
  }

  // For other sizes, extract bytes manually
  for (let i = 0; i < number; i++) {
    buffer[i] = Number((bigIntValue >> BigInt(8 * i)) & 0xFFn);
  }
...
export function readBufferFromBigInt(
  ...
  if (value === undefined || value === null) {
    throw new Error('readBufferFromBigInt: value is undefined or null');
  }
  ...
  if (bigIntValue < min || bigIntValue > max) {
    throw new RangeError(
      `Value ${bigIntValue} does not fit in ${bytesNumber} ${signed ? 'signed' : 'unsigned'} bytes`,
    );
  }

  // Two's complement encode if negative
  let v = signed && bigIntValue < 0n ? (1n << bits) + bigIntValue : bigIntValue;
...
export function bigIntMod(n: bigint | number | string, m: bigint | number | string) {
  const bigN = typeof n === 'bigint' ? n : BigInt(n);
  const bigM = typeof m === 'bigint' ? m : BigInt(m);
  return ((bigN % bigM) + bigM) % bigM;
}
...
export function modExp(
  ...
  let x = bigA;
  while (bigB > 0n) {
    const leastSignificantBit = bigB % 2n;
    bigB = bigB / 2n;
...
export function getByteArray(integer: bigint | number | string, signed = false): Buffer<ArrayBuffer> {
  const bigInteger = typeof integer === 'bigint' ? integer : BigInt(integer);
  if (!signed && bigInteger < 0n) {
    throw new RangeError('Cannot convert negative to unsigned');
  }
  ...
  if (signed) {
    if (bigInteger >= 0n) {
      const bits = bitLength(bigInteger) + 1;
      bytes = Math.max(1, Math.ceil(bits / 8));
    } else {
      const bits = bitLength(-bigInteger - 1n) + 1;
      bytes = Math.max(1, Math.ceil(bits / 8));
    }
  } else {
    const bits = bitLength(bigInteger);
    bytes = Math.max(1, Math.ceil(bits / 8));
  }

  return readBufferFromBigInt(bigInteger, bytes, false, signed);
}
...
export function bitLength(x: bigint | number | string) {
  const bigX = typeof x === 'bigint' ? x : BigInt(x);
  let k = 0;
  while (true) {
    if (testersN === k) {
    ...
    if (bigX < testers[k]) break;
    k++;
  }

  if (!k) return 32 - Math.clz32(Number(bigX));

  // determine length by bisection
  k--;
  let i = testersCoeff[k];
  let a = bigX >> testersBigCoeff[k];
  while (k--) {
