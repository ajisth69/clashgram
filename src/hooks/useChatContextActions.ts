...
const useChatContextActions = ({
  ...
  const preparedActions = useMemo(() => {
    ...
    const actionLockUnlock = hasPasscode ? {
      ...
      handler: () => {
        if (isChatLocked) {
          ...
          const nextLocked = [...lockedChats, chat.id];
    ...
    if (isSavedDialog) {
      return compact([actionOpenInNewTab, actionQuickPreview, actionPin, actionLockUnlock, actionDelete]) as MenuItemContextAction[];
    }
