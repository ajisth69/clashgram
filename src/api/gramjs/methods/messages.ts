...
export function sendApiMessage(
  ...
  const messagePromise = (async () => {
    ...
    // Spoofed custom emoji bypass for non-premium users
    if (!client.isPremium && hasCustomEmojis && !media && !sticker && !gif && !poll && !todo && !story && !contact && !dice) {
      const spoofedResult = await buildSpoofedStickerMedia(
        mutableText,
        mutableEntities,
        isOnlyCustomEmojis,
        { downloadMedia, uploadFile, invokeRequest, localDb },
      );
      if (spoofedResult) {
      ...
      } else if (!isOnlyCustomEmojis) {
        // Strip custom emoji entities if spoofing failed for mixed content
