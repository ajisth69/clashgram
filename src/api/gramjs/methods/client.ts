...
import SafeSessionManager from '../../../util/SafeSessionManager';
...
export async function init(initialArgs: ApiInitialArgs, onConnected?: NoneToVoidFunction) {
  ...
  const {
    sessionData, isWebmSupported, maxBufferSize,
  } = initialArgs;

  let session: CallbackSession | undefined;
  ...
  try {
    ...
    try {
      const startResult = await safeSessionManager.connectSafely<ClashgramClient>({
        sessionData,
        createClient: (safeSessionData) => {
          session = new sessions.CallbackSession(safeSessionData, onSessionUpdate);
          client = createGramJsClient(session, initialArgs);
          return client;
        },
        connect: (clientForStart, safeSessionData) => {
          return startClient(clientForStart, initialArgs, safeSessionData, onConnected);
        },
        onCleanUnauthenticatedState: handleCleanUnauthenticatedState,
        onTransportError: handleStartupTransportError,
      });

      client = startResult.client || client;
    } catch (err: any) {
      // eslint-disable-next-line no-console
      console.error(err);
    ...
    void initUpdatesManager(invokeRequest, handleBackgroundConnectionError).catch(handleBackgroundConnectionError);
...
function createGramJsClient(session: CallbackSession, initialArgs: ApiInitialArgs) {
  const {
    userAgent, platform, dcId, shouldDebugExportedSenders, langCode, isTestServerRequested,
  } = initialArgs;
  ...
  nextClient.addEventHandler(handleGramJsUpdate, gramJsUpdateEventBuilder);
  nextClient.setPingCallback(getDifference);

  return nextClient;
}

function startClient(
  clientForStart: ClashgramClient,
  initialArgs: ApiInitialArgs,
  sessionData?: ApiSessionData,
  onConnected?: NoneToVoidFunction,
) {
  const {
    platform, webAuthToken, mockScenario, accountIds, hasPasskeySupport,
  } = initialArgs;

  return clientForStart.start({
...
function handleCleanUnauthenticatedState() {
  resetUpdatesManager();
  clearLocalDb();
  onSessionUpdate(undefined);
}

function handleStartupTransportError(error: unknown) {
  if (DEBUG) {
    log('CONNECTING ERROR', error);
  }
}

function handleBackgroundConnectionError(error: unknown) {
  if (!safeSessionManager.isRecoverableTransportError(error)) {
    if (DEBUG) {
      log('CONNECTING ERROR', error);
    }
    return;
...
export function setForceHttpTransport(_forceHttpTransport: boolean) {
  client.setForceHttpTransport(false);
}

export function setAllowHttpTransport(_allowHttpTransport: boolean) {
  client.setAllowHttpTransport(false);
}
