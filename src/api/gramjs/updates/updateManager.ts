...
type BackgroundErrorHandler = (error: unknown) => void;
...
let isInited = false;
let backgroundErrorHandler: BackgroundErrorHandler | undefined;
...
export async function init(invokeReq: typeof invokeRequest, onBackgroundError?: BackgroundErrorHandler) {
  invoke = invokeReq;
  backgroundErrorHandler = onBackgroundError;
...
export async function getDifference() {
  ...
  if (response instanceof GramJs.updates.DifferenceSlice) {
    void getDifference().catch(handleBackgroundError);
    return;
  }
...
function forceSync() {
  ...
  void loadRemoteState().catch(handleBackgroundError);
}

function handleBackgroundError(error: unknown) {
  backgroundErrorHandler?.(error);
}
