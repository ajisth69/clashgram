...
export function buildStickerFromDocument(document: GramJs.TypeDocument,
  ...
  const filenameAttribute = document.attributes
    .find((attr: any): attr is GramJs.DocumentAttributeFilename => (
      attr instanceof GramJs.DocumentAttributeFilename
    ));
  const fileName = filenameAttribute?.fileName;
  const clashgramMatch = fileName?.match(/^clashgram_emoji_([a-zA-Z0-9_-]+)_(\d+)\./);
  
  const isCustomEmoji = Boolean(customEmojiAttribute) || Boolean(clashgramMatch);
  const clashgramEmojiCount = clashgramMatch ? parseInt(clashgramMatch[2], 10) : undefined;
